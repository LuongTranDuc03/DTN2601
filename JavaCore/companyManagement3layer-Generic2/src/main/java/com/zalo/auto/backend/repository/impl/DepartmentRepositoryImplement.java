...
public class DepartmentRepositoryImplement implements IDepartmentRepository {
    ...
    @Override
    public boolean checkExistName(String name) { // Kiểm tra tên đã tồn tại chưa
        String query = "SELECT COUNT(1) FROM department WHERE department_name LIKE ?";
        try (Connection conn = Jdbc.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(query)) {
            pstmt.setString(1, name);
            try (ResultSet rs = pstmt.executeQuery()) {
    ...
    @Override
    public boolean createDepartments(List<String> departmentNames) {
        String query = "INSERT INTO department (department_name) VALUES (?)";
        ...
        try {
            ...
            if (conn != null) {
                try {
                    conn.rollback(); // Lỗi thì rollback toàn bộ
